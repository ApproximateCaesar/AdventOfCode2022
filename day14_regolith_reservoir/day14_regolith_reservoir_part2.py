...
max_y_rock = max([int(p[1]) for p in rock])  # maximum y pos of a rock
rock.update(get_line_of_points((-1000, max_y_rock + 2), (1000, max_y_rock + 2)))
print_cave(rock, sand)

# can the while loop logic be simplified?
source_blocked = False  # whether sand has blocked the source point
i = 0
while not source_blocked:
    ...
    obstacles = rock.union(sand)
    if (500, 0) in sand:  # sand blocking source
        source_blocked = True
    else:
        while not at_rest:
            if (sand_unit_pos[0], sand_unit_pos[1] + 1) not in obstacles:
